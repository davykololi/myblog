<template>
  <Head>   
    ...
    <guest-layout>
      <template #header>
        <Breadcrumb :routelink="['about-us',]" :text="['About',]"></Breadcrumb>
      </template>

      <SchemaOrg :schemaorg="aboutUsSchema" v-memo/>

      <main-page>
          <h2 class="h2-gradient" v-once>{{ title }}</h2>
      </main-page>
...
const aboutUsSchema = computed(() => props.aboutSchemaOrg);
