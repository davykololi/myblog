            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200 dark:bg-gray-700 dark:text-slate-400">
                        <div class="mb-12">
                            <Link :href="route('editor.articles.index')">
                        ...
                        <form @submit.prevent="submit" enctype="multipart/form-data">
                            ...
                            <div class="sm:col-span-3">
                                <label for="published_at" class="block text-sm font-medium leading-6 text-gray-900">Published At</label>
                                <VueDatePicker v-model="form.published_at" class="dark:bg-stone-600"/>
                            </div>

                            <div class="sm:col-span-3">
                                <label for="published" class="block text-sm font-medium leading-6 text-gray-900">Publish</label>
                                <div class="flex flex-row space-x-16">
                                    <span>
                                        Yes <input id="is_published" name="is_published" type="radio" v-model="form.is_published" value="1"/>
                                    </span>
                                    <span>
                                        No <input id="is_published" name="is_published" type="radio" v-model="form.is_published" value="0"/>
                                    </span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div>
                                    <select name="user_id" id="user_id" class="min-w-max text-sm text-gray-900 bg-transparent border-2 rounded-xl" v-model="form.user_id">
                                        <option v-for="user in authors" :key="user.id" :value="user.id" selected>
                                            {{ user.name }}
                                        </option>
                                    </select>
                                </div>
...
const submit = () => {
    ...
    form.post(route('editor.articles.update', {id: props.article.id}), {
        body: formData
    });
}
